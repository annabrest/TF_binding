...
##-- Plot heatmap for differnt thresholds for % of overlaps:

```{r}

#Function to plot heatmaps of data after converting probability of overlap X to be 100%
heatmap_filtered_data <- function(overlap_par, data, Path_to_plot, Title_plot, Rowv_arg, Colv_arg, RowSideColors_arg, ColSideColors_arg ){
  
  data_plot <- hclust_data %>% 
  mutate_all(.,funs(if_else(.> overlap_par, 100, 0)))

jpeg(paste0(Path_to_plot, Title_plot,"_Rowv_Colv_TF", "_overlap_", as.character(overlap_par), ".jpeg"))
par(cex.main=1) #controls the size of the title
                ...
                heatmap.2(as.matrix(data_plot),
                          ...
                          labRow=NA,  
                          labCol=NA,
                          main=paste(Title_plot,"_overlap_", as.character(overlap_par)),
                          RowSideColors = RowSideColors_arg, 
                          ColSideColors = ColSideColors_arg, 
                          key=TRUE,
                          #density.info = "none",  #remove the hist from key
                          #lhei = c(2, 8), # this makes the colour-key legend a little thinner,
                          #sepwidth=c(0.05,0.05),
...
#Parameters:
overlap_list <- c(12.5, 25, 50, 75, 90)
Colv <- hclust_columns$Colv_arg 
Rowv <-   hclust_rows$Rowv_arg
RowSideColors <- hclust_rows$RowSideColors_arg
ColSideColors <- hclust_columns$ColSideColors_arg

lapply(overlap_list, function(x)heatmap_filtered_data(overlap_par = x, data = hclust_data, Path_to_plot, Title_plot, Rowv_arg = Rowv, Colv_arg = Colv, RowSideColors_arg = RowSideColors, ColSideColors_arg = ColSideColors))
...
Clust %>% 
group_by(Cluster_rows, Cluster_columns) %>% count(TF_family)

test <- Clust %>% 
group_by(Cluster_rows) %>% count(Cluster_columns) %>% arrange(Cluster_rows, n)
...
Clust %>% filter(Cluster_rows == 19, Cluster_columns == 17) %>% 
  group_by(TF_family) %>% count()
...
Clust %>% 
   filter(motif == "ABI3VP1_tnt.REM16_col_a_m1" | motif ==  "ARF_tnt.ARF16_col_b_m1" | motif ==  "E2FDP_tnt.DEL2_col_a_m1" |
            motif == "E2FDP_tnt.DEL1_colamp_a_m1" | motif == "E2FDP_tnt.DEL2_colamp_a_m1" | motif ==  "E2FDP_tnt.DEL1_col_a_m1")
          

top_TF_rf_hclust <- data_matrix %>% 
  filter(motif_1 == "ABI3VP1_tnt.REM16_col_a_m1" | motif_1 ==  "ARF_tnt.ARF16_col_b_m1" | motif_1 ==  "E2FDP_tnt.DEL2_col_a_m1" |
            motif_1 == "E2FDP_tnt.DEL1_colamp_a_m1" | motif_1 == "E2FDP_tnt.DEL2_colamp_a_m1" | motif_1 ==  "E2FDP_tnt.DEL1_col_a_m1") %>% 
  select(motif_1, "ABI3VP1_tnt.REM16_col_a_m1", "ARF_tnt.ARF16_col_b_m1", "E2FDP_tnt.DEL2_col_a_m1",	"E2FDP_tnt.DEL1_colamp_a_m1", "E2FDP_tnt.DEL2_colamp_a_m1", "E2FDP_tnt.DEL1_col_a_m1") 

test <- data_matrix_cluster_reordered %>% 
  filter(cluster_rows == 2 | cluster_rows == 3) %>% 
  select(motif_1, names(hclust_columns$Cluster_columns[hclust_columns$Cluster_columns == 1]))
...
E2FDP_tnt.DEL2_col_a_m1,			
E2FDP_tnt.DEL1_colamp_a_m1,			
E2FDP_tnt.DEL2_colamp_a_m1,			
E2FDP_tnt.DEL1_col_a_m1	
